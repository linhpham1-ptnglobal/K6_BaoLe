...
const loadExternalReportWelshV2 = (data) => {
    console.log('Doing loadExternalReportWelshV2');

    const reportUrl = 'http://sqlreports3/ReportServer/Pages/ReportViewer.aspx?%2FHMCTSQA-AZURE%2FExternalHearingList_Welsh_v2';

    let res = http.get(reportUrl);

    check(res, {'External Report Welsh V2 screen load succeeded': res => res.url == reportUrl})
    //  || fail('External Report Welsh V2 screen load failed' + res.url);

    console.log(res)

    /** LOAD EXTERNAL REPORT WELSH V2 RESULT */
    res = res.submitForm({
        formSelector: 'form',
        fields: {
    ...
    check(res, {'External Report Welsh V2 load succeeded': res => res.url == reportUrl}) 
    // || fail('External Report Welsh V2 load failed' + res.url);
}

export { loadExternalReportWelshV2 }